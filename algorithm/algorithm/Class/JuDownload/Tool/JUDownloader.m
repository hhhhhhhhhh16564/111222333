...
static dispatch_queue_t url_dowload_manager_creation_queue() {
    static dispatch_queue_t download_creation_queue;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        download_creation_queue = dispatch_queue_create("com.july.edu.algorithm", DISPATCH_QUEUE_SERIAL);
    });
    
    return download_creation_queue;
}
 ...
 NSInteger length=[[[fileManager attributesOfItemAtPath:destinationPath error:nil] objectForKey:NSFileSize] integerValue];
    ...
    if(urlString&&destinationPath)
    {
        ...
        if ([urlString hasSuffix:@".mp4"]) {
            ...
            dispatch_queue_t queue1 = url_dowload_manager_creation_queue();
