...
-(void)setEnableEdgeInsetsWithTop:(CGFloat)top left:(CGFloat)left bottom:(CGFloat)bottom right:(CGFloat)right{
    
    objc_setAssociatedObject(self, &topNameKey, @(top), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    objc_setAssociatedObject(self, &leftNameKey, @(left), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    objc_setAssociatedObject(self, &bottomNameKey, @(bottom), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    objc_setAssociatedObject(self, &rightNameKey, @(right), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
-(CGRect)enableRect{
    ...
    CGFloat top  = [objc_getAssociatedObject(self, &topNameKey) floatValue];
    CGFloat left  = [objc_getAssociatedObject(self, &leftNameKey) floatValue];
    CGFloat bottom  = [objc_getAssociatedObject(self, &bottomNameKey) floatValue];
    CGFloat right  = [objc_getAssociatedObject(self, &rightNameKey) floatValue];
...
-(BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event{
    ...
    CGRect enableRect = [self enableRect];
    
    if (CGRectEqualToRect(enableRect, self.bounds)) {
